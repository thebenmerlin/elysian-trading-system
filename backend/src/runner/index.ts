...
export class TradingRunner {
  ...
  private async runAIAnalysisPhase(): Promise<void> {
    ...
    try {
      ...
      const features = await featuresEngine.getLatestFeatures(this.currentCycle!.tickers_processed);
      const signals = await signalEngine.getLatestSignals(this.currentCycle!.tickers_processed, 10);

      let analysesCompleted = 0;

      for (const featureSet of features) {
        try {
          const marketData = await dataIngestor.getHistoricalData(featureSet.symbol, 7);
          const symbolSignals = signals.filter(s => s.symbol === featureSet.symbol);

          if (marketData.length > 0) {
            await aiReasoner.analyzeMarket(featureSet.symbol, marketData, featureSet, symbolSignals);
            analysesCompleted++;
          }
        } catch (error) {
          logger.warn(`AI analysis failed for ${featureSet.symbol}:`, error);
        }
      }
  ...
  private async runTradeExecutionPhase(): Promise<void> {
    ...
    try {
      ...
      const signals = await signalEngine.getLatestSignals(this.currentCycle!.tickers_processed, 20);
      const aiAnalyses = await aiReasoner.getLatestAnalysis(this.currentCycle!.tickers_processed);
      const latestPortfolio = await portfolioManager.getLatestPortfolioSnapshot();
      ...
      for (const signal of signals) {
        try {
          ...
          const aiAnalysis = aiAnalyses.find(ai => 
            ai.symbol === signal.symbol &&
            Math.abs(ai.timestamp.getTime() - signal.timestamp.getTime()) < 30 * 60 * 1000 // Within 30 minutes
          );

          if (aiAnalysis) {
            const trade = await executionEngine.evaluateAndExecute(signal, aiAnalysis, portfolioValue);
            if (trade) {
              tradesExecuted++;
            }
          } else {
            logger.debug(`No AI analysis found for ${signal.symbol} signal`);
          }
        } catch (error) {
          logger.warn(`Trade execution failed for ${signal.symbol}:`, error);
        }
      }
  ...
  private async runPortfolioUpdatePhase(): Promise<void> {
    ...
    try {
      ...
      const previousSnapshot = await portfolioManager.getLatestPortfolioSnapshot();
  ...
  private async runReflectionPhase(): Promise<void> {
    ...
    try {
      ...
      const reflection = await reflectionEngine.generateReflection(7); // Last 7 days
  ...
  private async runReportingPhase(): Promise<void> {
    ...
    try {
      ...
      const report = await reportsGenerator.generatePerformanceReport(30); // Last 30 days
  ...
  async getRunHistory(limit: number = 20): Promise<RunCycle[]> {
    try {
      const query = `
  ...
  updateConfiguration(newConfig: Partial<RunnerConfig>): void {
    this.config = { ...this.config, ...newConfig };
    logger.info('Runner configuration updated', newConfig);
  }
}

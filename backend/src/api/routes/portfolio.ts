...
router.get('/current', async (req, res) => {
  try {
    ...
    logger.error('Failed to get current portfolio:', error);
    res.status(500).json({
...
router.get('/history', async (req, res) => {
  try {
    ...
    const history = await portfolioManager.getPerformanceHistory(days);
...
router.get('/positions', async (req, res) => {
  try {
    const positions = await portfolioManager.getCurrentPositions();
...
router.get('/cash', async (req, res) => {
  try {
    const cash = await portfolioManager.getCashBalance();
...
// Create portfolio snapshot (manual trigger)
router.post('/snapshot', async (req, res) => {
  try {
    const snapshot = await portfolioManager.createPortfolioSnapshot();

...
class DataIngestor {
    ...
    async initializeCryptoPairsAsync() {
        try {
            setTimeout(async () => {
            ...
            logger_1.logger.warn('Failed to initialize crypto pairs, will retry later:', error);
            this.cryptoPairs = [
    ...
    async initializeCryptoPairs() {
        ...
        try {
            ...
            logger_1.logger.info(`📋 Loaded ${this.cryptoPairs.length} active crypto pairs`);
            this.initialized = true;
        }
        catch (error) {
            logger_1.logger.error('Failed to load crypto pairs:', error);
            this.cryptoPairs = [
                { symbol: 'BTCUSDT', base_asset: 'BTC', quote_asset: 'USDT', min_trade_amount: 0.00001, price_precision: 2, quantity_precision: 5 },
                { symbol: 'ETHUSDT', base_asset: 'ETH', quote_asset: 'USDT', min_trade_amount: 0.001, price_precision: 2, quantity_precision: 4 },
    ...
    async fetchMarketData(symbols, marketType = 'equity') {
        const marketData = [];
        logger_1.logger.info(`📊 Fetching ${marketType} market data for ${symbols.length} symbols`);
            ...
            try {
                ...
                if (data) {
                    ...
                    try {
                        ...
                        logger_1.logger.debug(`✅ Stored ${marketType} data for ${symbol}: $${data.close.toFixed(data.market_type === 'crypto' ? 2 : 2)}`);
                    }
                    catch (storeError) {
                        logger_1.logger.warn(`⚠️ Failed to store data for ${symbol}:`, storeError.message);
                    }
                }
            }
            catch (error) {
                logger_1.logger.error(`Failed to fetch ${marketType} data for ${symbol}:`, error.message);
        ...
        logger_1.logger.info(`📊 ${marketType.toUpperCase()} data fetch complete: ${marketData.length}/${symbols.length} successful`);
        return marketData;
    }
        ...
        try {
            ...
            for (const url of binanceUrls) {
                try {
                    ...
                    lastError = error;
                    logger_1.logger.debug(`Binance endpoint ${url} failed:`, error.message);
                    continue;
                }
            }
            if (!response || !response.data || !response.data.symbol) {
                logger_1.logger.warn(`All Binance endpoints failed for ${symbol}, using fallback data`);
                return this.generateCryptoFallbackData(symbol);
            }
            const data = response.data;
            let ohlcData = null;
            try {
                const klineResponse = await axios_1.default.get(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=1d&limit=2`, { timeout: 5000 });
                ...
                logger_1.logger.debug(`Kline data failed for ${symbol}, using ticker data`);
            }
            const marketData = {
                symbol,
            ...
            if (marketData.close <= 0 || marketData.volume < 0) {
                logger_1.logger.warn(`Invalid data for ${symbol}, using fallback`);
                return this.generateCryptoFallbackData(symbol);
            }
            if (marketData.high < marketData.low) {
                logger_1.logger.warn(`Invalid OHLC for ${symbol}, using fallback`);
                return this.generateCryptoFallbackData(symbol);
            }
            logger_1.logger.debug(`🪙 Real crypto data for ${symbol}: $${marketData.close.toFixed(2)} (Vol: ${marketData.volume.toFixed(0)})`);
            return marketData;
        }
        catch (error) {
            logger_1.logger.error(`Crypto fetch completely failed for ${symbol}:`, error.message);
            return this.generateCryptoFallbackData(symbol);
        }
    }
    generateCryptoFallbackData(symbol) {
        const cryptoPrices = {
        ...
        logger_1.logger.info(`🎲 Generated fallback crypto data for ${symbol}: $${close.toFixed(2)}`);
        return {
            symbol,
            timestamp: new Date(),
    ...
    async fetchEquityData(symbol) {
        try {
            ...
            for (const endpoint of endpoints) {
                try {
                    response = await axios_1.default.get(endpoint, {
                        headers: {
            ...
            logger_1.logger.debug(`📈 Equity data fetched for ${symbol}: $${marketData.close.toFixed(2)}`);
            return marketData;
        }
        catch (error) {
            logger_1.logger.error(`Yahoo Finance fetch failed for ${symbol}:`, error.message);
            return null;
        }
    }
    ...
    async getFallbackData(symbol, marketType) {
        try {
            const query = `
        ...
        LIMIT 1
      `;
            const result = await database_1.DatabaseManager.query(query, [symbol, marketType]);
            if (result.rows.length > 0) {
                ...
                logger_1.logger.info(`📈 Using fallback ${marketType} data for ${symbol}: $${newPrice.toFixed(2)}`);
                return {
                    symbol,
                    timestamp: new Date(),
                    open: newPrice * 0.999,
            ...
            logger_1.logger.error(`Fallback data failed for ${symbol}:`, error);
            return this.generateMockData(symbol, marketType);
        }
    }
    generateMockData(symbol, marketType) {
        const basePrices = {
        ...
        logger_1.logger.debug(`🎲 Mock ${marketType} data generated for ${symbol}: $${close.toFixed(2)}`);
        return {
            symbol,
            timestamp: new Date(),
    ...
    async getHistoricalData(symbol, days, marketType) {
        try {
            let query = `
        SELECT * FROM market_data 
    ...
    async isMarketOpen(marketType) {
        try {
            ...
            logger_1.logger.error('Failed to check market hours:', error);
            return false;
        }
    }
}

...
class PortfolioManager {
    async getLatestPortfolioSnapshot() {
      ...
      `;
            ...
            logger_1.logger.error('Failed to get latest portfolio snapshot:', error);
            return null;
        }
    }
    async getPortfolioHistory(days) {
        try {
            const query = `
        SELECT * FROM portfolio_snapshots 
    ...
    async createPortfolioSnapshot() {
        try {
            ...
            const currentValue = initialCash + (Math.random() - 0.5) * 1000;
            const dailyPnl = currentValue - previousValue;
            const snapshot = {
                timestamp: new Date(),
                total_value: currentValue,
                cash: currentValue * 0.8,
                positions_value: currentValue * 0.2,
